...
def bench_participation_set_update(n_list, m, alpha, beta):
    ...
    for n in n_list:
        ...
        active_set_update(m, n, alpha, beta, q)
        stop_timer(i+3)    
...
def model_input_commitment(n, d_list, alpha, beta, m):
    ...
    for d in d_list: 
        ...
        for p in range(m):        
            ...
            # multiplty  and d^2 matrices with linear combination
            mat_prod(matrix_sa, matrix_sb)
            # multiplty  d \times n  \times n \times n diag with linear comb
            matrix_k = generate_random_shared_matrix(1, n, alpha, beta) 
            matrix_l = generate_random_shared_matrix(n, 1, alpha, beta) 
            mat_prod(matrix_k, matrix_l)
            # multiply d \times n \times n \times d  with linear comb on both sides
            mat_prod(matrix_k, matrix_l)
            # chekc inverse
            check_inverse(d, matrix_a, 0.01)
